...
def get(url, headers=None, encoding='UTF-8'):
    ...
    try:
        ...
        return "ERROR:" + logging.exception(e)
    except ConnectTimeout as e:
        return "ERROR:" + "HTTP连接超时错误"+ logging.exception(e)
...
def get_stream(url, headers=None, encoding='UTF-8'):
    ...
    try:
        ...
        return "ERROR:" + "HTTP连接错误"+ logging.exception(e)
    except ConnectTimeout as e:
        return "ERROR:" + "HTTP连接超时错误"+ logging.exception(e)
    except Exception as e:
        return 'ERROR:' +  logging.exception(e)
...
def post_stream(url, data=None, headers=None, encoding='UTF-8', files=None):
    """分块接受数据"""
    try:
        lines = requests.post(url, data=data, headers=headers, timeout=_tiemout, stream=True, proxies=proxies,verify=False,
                              files=None)
        html = list()
        for line in lines.iter_lines():
            line = line.decode(encoding)
...
def post_file(url, exp, headers=None, encoding='UTF-8'):
    ...
    try:
        ...
        return "ERROR:" + '1'+ logging.exception(e)
    except ConnectTimeout as e:
        return "ERROR:" + "HTTP连接超时错误"+ logging.exception(e)
    except Exception as e:
        return 'ERROR:' + str(e)+ logging.exception(e)
